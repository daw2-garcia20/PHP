...
//______________________________TABLA DE PEOPLE__________________________________
$tabla.= <<<ENDHTML
  <table>
    <tr>
      <th style="text-align:center;">peoples</th>
      <th colspan="2"><a href="people.php?action=add"><button>ADD</button></a></th>
    </tr>
ENDHTML;
  ...
  while ($row = mysqli_fetch_assoc($result)) {
    extract($row);
    $nombre = $people_fullname;
    $tabla .= <<<ENDHTML
      <tr>
        <td>$nombre</td>
        <td><a href="people.php?action=edit&id=$people_id"><button>EDIT</button></a></td>
        <td><a href="delete.php?type=people&id=$people_id"><button>DELETE</button></a></td>
      </tr>
ENDHTML;
  }
  echo $tabla;
